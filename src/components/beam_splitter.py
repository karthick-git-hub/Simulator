...
class BeamSplitter(Entity):
    ...
    def __init__(self, name: str, timeline: "Timeline", fidelity=1, is_cow_protocol=False, data_line_probability=0.7):
        ...
        # for COW
        self.is_cow_protocol = is_cow_protocol
        self.data_line_probability = data_line_probability
        # for BB84
        self.start_time = 0
        self.frequency = 0
        self.basis_list = []
    ...
    def get(self, photon, **kwargs) -> None:
        ...
        if self.get_generator().random() < self.fidelity:
            if not self.is_cow_protocol:
                assert photon.encoding_type[
                           "name"] == "polarization", "Beamsplitter should only be used with polarization."
                ...
                # COW protocol specific behavior
                if self.get_generator().random() < self.data_line_probability:
                    self._receivers[0].getPhoton(photon)  # Assuming first receiver is data line
                else:
                    self._receivers[1].getPhoton(photon)  # Assuming second receiver is monitoring line
