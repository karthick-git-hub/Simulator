...
class LightSource(Entity):
    ...
    def custom_emit(self, state_list) -> None:
        """Custom method to emit photons for the COW protocol.

        Arguments:
            state_list (List[List[complex]]): list of complex coefficient arrays to send as photon-encoded qubits.
        """
        log.logger.info("{} custom emitting {} photons".format(self.name, len(state_list)))
